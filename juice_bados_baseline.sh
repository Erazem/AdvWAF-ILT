...
# v3.0 by F5 Training Development Group
                        ...
                        while true; do
                                ...
                                for i in $(eval echo "{0..`date +%M`}")
                                        do
                                                ...
                                                curl -0 --interface $SRC_ADDR2 -s -o /dev/null -A "`shuf -n 1 useragents_with_bots.txt`"  http://$IP`shuf -n 1 urls.txt`
                                                curl -0 --interface $SRC_ADDR3 -s -o /dev/null -A "`shuf -n 1 useragents_with_bots.txt`"  http://$IP`shuf -n 1 urls.txt`
                                ...
                                if (( `date +%k` % 2 )); then
                                        ...
                                        for i in {1..50};
                                                do
                                                        ...
                                                        curl --interface $SRC_ADDR2 -s -o /dev/null -A "`shuf -n 1 useragents_with_bots.txt`"  http://$IP`shuf -n 1 urls.txt`
                                                        curl --interface $SRC_ADDR3 -s -o /dev/null -A "`shuf -n 1 useragents_with_bots.txt`"  http://$IP`shuf -n 1 urls.txt`
                                                        #curl -0 -s -o /dev/null -A "`sort -R useragents_with_bots.txt | head -1`" -w "status: %{http_code}\tbytes: %{size_download}\ttime: %{time_total}\n" http://$IP`sort -R urls.txt | head -1`
                                                done
                                fi
                                #sleep 0.1
